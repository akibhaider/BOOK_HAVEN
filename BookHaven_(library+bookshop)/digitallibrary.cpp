    ...
    ui(new Ui::digitalLibrary)
{
    ui->setupUi(this);
    connectDB();
    this->setWindowTitle("Admin Control Panel");
...
void digitalLibrary::connectDB()
{
    ...
    //Create a table named admin_accounts
    QString accountTable{"CREATE TABLE IF NOT EXISTS admin_accounts"
                         "(username VARCHAR(20), name VARCHAR(20),"
                         "password VARCHAR(20))"};


    if(!query.exec(accountTable))
        QMessageBox::critical(this,"Info","Cannot create admin_accounts");
...
void digitalLibrary::on_returnBookBtn_4_clicked()
{
    this->hide();
    customer_login cl;
    cl.exec();
